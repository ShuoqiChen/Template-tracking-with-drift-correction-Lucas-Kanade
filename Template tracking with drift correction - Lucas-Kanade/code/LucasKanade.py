...
def LucasKanade(It, It1, rect, p0):
    ...
    w = np.round(rect[2] - rect[0]) + 1  # width of patch
    h = np.round(rect[3] - rect[1]) + 1  # height of patch
    w = w.astype(int)
    h = h.astype(int)
    ...
    # Use this:
    # ------------------------------------------------------------
    # tmplt_0 = It[rect[1]:rect[3] + 1, rect[0]:rect[2] + 1]
    ...
    y_vct = np.linspace(rect[1], rect[3], num=h)
    ...
    tmplt = tmplt_spline.ev(y_mtx, x_mtx)  # interpolate (get the pixel intensity) of the input patch
    #                                                           # ...after being warped


    # Common initialisation
    # P0 starts with all 0s
    tmplt_pts, warp_p = init_affine(p0=p0, rect=rect)
    ...
    while new_rms_error > threshold:
        ...
        IWxp = warp_affine(img=It1, p=warp_p, tmplt=tmplt, rect=rect)
        ...
        nabla_Ix = warp_affine(img=img_dx, p=warp_p, tmplt=tmplt, rect=rect)
        nabla_Iy = warp_affine(img=img_dy, p=warp_p, tmplt=tmplt, rect=rect)
...
def LucasKanade_template_drift(It0, It1, rect0, rect1, p0):
    # Input: 
    #   It: template image
    #   It1: Current image
    #   rect: Current position of the car
    ...
    w = np.round(rect1[2] - rect1[0]) + 1  # width of patch
    h = np.round(rect1[3] - rect1[1]) + 1  # height of patch
    w = w.astype(int)
    h = h.astype(int)


    # tmplt1 = It1[rect1[1]:rect1[3]+1, rect1[0]:rect1[2]+1]
    tmplt_spline = RectBivariateSpline(np.array(range(0, It1.shape[0])), np.array(range(0, It1.shape[1])), It1)  # img should be the template image

    # create x and y coordinates in subpixel level
    x_vct = np.linspace(rect1[0], rect1[2], num=w)
    y_vct = np.linspace(rect1[1], rect1[3], num=h)
    ...
    tmplt1 = tmplt_spline.ev(y_mtx, x_mtx)  # interpolate (get the pixel intensity) of the input patch
    #                                                           # ...after being warped
    ...
    It1_w = It1_w.astype(int)
    It1_h = It1_h.astype(int)
...
# Not sure if the following function is correct of not....
...
def warp_affine(img, p, tmplt, rect):
    ...
    x_array = np.linspace(rect[0], rect[2], num=w)
    y_array = np.linspace(rect[1], rect[3], num=h)


    for i in range(h):  # for each row
        for j in range(w): # for each col
    ...
    #     for j in range(rect[0], rect[2]): # for each col
            ...
            xy = np.matmul(W, np.asarray([[x_coord],[y_coord],[1]]))   # the wrapped coordinates

            x = xy[1]  # col index, corresponding to x 
            y = xy[0]  # row index, corresponding to y
    
            wimg[i][j] = rect_spline.ev(x,y)  # interpolate (get the pixel intensity) of the input patch
                                                              # ...after being warped
