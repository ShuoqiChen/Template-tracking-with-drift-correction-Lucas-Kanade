...
def LucasKanadeBasis(It, It1, rect, bases):
    ...
    while new_rms_error > threshold:
        ...
        # Initialize and put in the first base
        B= bases[:,:,0]

        for i in range(1, bases.shape[2]):
            B = np.hstack((B,bases[:,:,i]))

        # print(B.shape)
        # print(error_img.shape)
        error_img = (error_img - np.matmul(np.matmul(B, B.T), error_img))
        # ------------------------------------------------------------------------
        ...
        # However, what is B!!! How to make a linear combination of B???
        # Initialize and put in the first base
        B= bases[:,:,0]
        ...
        VI_dW_dp = (VI_dW_dp - np.matmul(np.matmul(B, B.T), VI_dW_dp))
        # ------------------------------------------------------------------------
